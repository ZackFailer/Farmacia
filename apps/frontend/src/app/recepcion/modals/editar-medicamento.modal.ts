...
@Component({
  ...
  imports: [
    IonHeader, IonToolbar, IonTitle, IonButtons, IonButton,
    IonContent, IonItem, IonLabel, IonInput, IonSelect, IonSelectOption,
    IonToggle, IonFooter, FormsModule,
  ],
  template: `
    <ion-header>
      <ion-toolbar>
        <ion-title>Editar Medicamento</ion-title>
...
export class EditarMedicamentoModal implements OnInit {
  ...
  guardar() {
    if (!this.puedeGuardar()) {
      this.errorFormulario.set('Complete nombre, presentación y concentración para continuar.');
      return;
    }
